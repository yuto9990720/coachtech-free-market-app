...
items
  id, user_id(FK→users), condition_id(FK→conditions),
  name, brand, description, price, image, is_sold,
  created_at, updated_at

item_categories
  id, item_id(FK→items), category_id(FK→categories)

purchases
  id, user_id(FK→users), item_id(FK→items),
  payment_method, postal_code, address, building,
  created_at, updated_at
